...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Generate JWT
        const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, JWT_SECRET, { expiresIn: '1h' } // Token expires in 1 hour
        );
...
app.post('/products', authorize(['manager', 'admin']), [
...
app.put('/products/:productId', authorize(['manager', 'admin']), async (req, res) => {
...
app.delete('/products/:productId', authorize(['manager', 'admin']), async (req, res) => {
    const productId = req.params.productId;
    try {
...
app.get('/clients', authorize(['sales', 'manager', 'admin']), async (req, res) => {
    try {
...
app.get('/clients/:clientId', authorize(['sales', 'manager', 'admin']), async (req, res) => {
    try {
        const client = await getClientById(db, req.params.clientId);
        if (client)
            res.json(client);
        else
...
app.post('/clients', authorize(['manager', 'admin']), async (req, res) => {
...
app.put('/clients/:clientId', authorize(['manager', 'admin']), async (req, res) => {
...
app.delete('/clients/:clientId', authorize(['manager', 'admin']), async (req, res) => {
    const clientId = req.params.clientId;
    try {
...
app.get('/cart', (req, res) => res.json(sessionCart));
app.post('/cart/add', async (req, res) => {
    const { productId, quantity } = req.body;
    try {
        const product = await getProductById(db, productId); // Pass db
        if (!product)
...
app.delete('/cart/item/:productId', (req, res) => {
    sessionCart = removeFromCart(sessionCart, req.params.productId);
    res.status(200).json(sessionCart);
});
app.put('/cart/item/:productId', (req, res) => {
    const { quantity } = req.body;
    sessionCart = updateItemQuantity(sessionCart, req.params.productId, quantity);
    res.status(200).json(sessionCart);
});
// --- Routes pour les ventes ---
app.get('/sales', authorize(['manager', 'admin']), async (req, res) => {
    try {
...
app.post('/sales', authorize(['sales', 'manager', 'admin']), async (req, res) => {
    const { paymentMethod, clientId } = req.body;
    if (!paymentMethod) {
...
app.post('/returns', authorize(['sales', 'manager', 'admin']), async (req, res) => {
    const { productId, quantity } = req.body;
    ...
    try {
        ...
        await updateProductStock(db, productId, quantity); // Incrémente le stock // Pass db
...
app.put('/users/:userId/role', authorize(['admin']), async (req, res) => {
...
app.put('/users/:userId/password', authorize(['admin']), async (req, res) => {
    const userId = req.params.userId;
...
app.delete('/users/:userId', authorize(['admin']), async (req, res) => {
    const userId = req.params.userId;
    try {
