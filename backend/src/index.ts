...
app.get('/stores', authorize(['manager', 'admin']), async (req: Request, res: Response) => {
  try {
...
app.get('/stores/:id', authorize(['manager', 'admin']), async (req: Request, res: Response) => {
  try {
    const store = await getStoreById(db, req.params.id as string);
    if (store) res.json(store);
    else res.status(404).send('Store not found');
  } catch (error) {
...
app.post('/stores', authorize(['admin']), async (req: Request, res: Response) => {
...
app.put('/stores/:storeId', authorize(['admin']), async (req: Request, res: Response) => {
...
app.delete('/stores/:storeId', authorize(['admin']), async (req: Request, res: Response) => {
  const storeId = req.params.storeId as string;
  try {
